...
export default function Game6({ gridSize, timer: initialTimer }: Game6Props) {
  ...
  const handleFlip = (index: number) => {
    if (isLocked || cards[index].flipped || cards[index].matched || isComplete || timer <= 0) return;
    ...
    return (
      <div className="p-6 text-center">
        <div className="bg-white rounded-xl shadow p-8 max-w-xl mx-auto">
          ...
          <p className="text-lg">
            ✅ Số thẻ đã ghép đúng: {cards.filter((c) => c.matched).length}/{cards.length}
          </p>
          <p className="text-lg mt-2">{win ? "🎯 Bạn đã ghép đúng tất cả các cặp thẻ!" : "💡 Hãy thử lại để hoàn thành tất cả các cặp thẻ nhé!"}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="hide-navbar-footer relative p-6 max-w-6xl mx-auto bg-gradient-to-b from-slate-800 to-slate-700 h-full rounded-xl shadow-lg">
      <div className="absolute top-4 left-4 w-16 h-16 z-10 bg-white rounded-full shadow-lg flex items-center justify-center">
        <CircularProgressbarWithChildren
          ...
          <div className={`text-sm font-bold ${timer <= 3 ? "text-red-600" : "text-blue-600"}`}>{timer}s</div>
        </CircularProgressbarWithChildren>
      </div>

      <div className={`grid ${gridClass} gap-3 pt-0`}>
        {cards.map((card, index) => (
          <div key={card.id} className="w-full aspect-square perspective" onClick={() => handleFlip(index)}>
            <div
              ...
              <div className="absolute inset-0 rotate-y-180 backface-hidden">
                <img src={card.image} alt={card.name} loading="lazy" className="w-full h-full object-cover rounded-lg" />
              </div>
            </div>
          </div>
        ))}
