    ...
    return (
      <div className="p-6 text-center text-yellow-100 h-full bg-[url('/image/pirate-wood.jpg')] bg-cover flex items-center justify-center font-pirate">
        <div className="bg-black/60 backdrop-blur p-8 rounded-xl max-w-xl w-full space-y-4 border-4 border-yellow-600 shadow-xl">
          ...
          <button onClick={restartGame} className="mt-4 bg-green-600 hover:bg-green-700 px-6 py-2 rounded-lg text-white shadow">
            🏁 Tiếp tục
          </button>
        </div>
      </div>
    ...
    <div className="flex w-full h-full gap-4 pt-4 px-6 bg-[#1e293b] font-pirate text-yellow-100">
        ...
        {places.map((place) => (
          <motion.div
            ...
            className={`absolute w-5 h-5 rounded-full cursor-pointer border-2
							${started && place.id === highlightId ? "bg-yellow-300 ring-2 ring-yellow-300 border-yellow-900" : "bg-red-600 border-yellow-100"}
      ...
      <div className="flex flex-col gap-4 w-[360px]">
        ...
        <div className="bg-[#1e293b]/90 border-4 border-yellow-600 p-4 rounded-lg shadow text-center">
          <h2 className="text-lg font-bold text-yellow-300 mb-2">🪙 Mảnh ghép cổ vật</h2>
          <div className="w-24 h-24 mx-auto relative">
            <CircularProgressbarWithChildren
              ...
              {hiddenProgress >= 100 ? (
                <button onClick={() => setShowPopup(true)} className="w-12 h-12 rounded-full bg-transparent hover:scale-110 transition">
                  <img src="/image/hidden-artifact.png" alt="Hidden Object" className="w-full h-full" />
                </button>
              ) : (
                <span className="text-yellow-200 text-sm font-bold">{Math.round(hiddenProgress)}%</span>
              )}
            </CircularProgressbarWithChildren>
          </div>
        </div>
        ...
        <div className="bg-[#1e293b]/90 border-4 border-yellow-600 p-4 rounded-lg shadow text-center">
          {started ? (
            ...
            <button onClick={handleStart} className="bg-yellow-600 hover:bg-yellow-700 px-6 py-2 rounded-lg text-white shadow">
              🏴‍☠️ Bắt đầu săn báu vật!
            </button>
          )}
          {waitingToFinish && (
            <button onClick={() => setShowConclusion(true)} className="mt-4 bg-green-600 hover:bg-green-700 px-6 py-2 rounded-lg text-white shadow">
              🏁 Xem kết quả
            </button>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-yellow-100 text-center text-black rounded-2xl p-6 max-w-sm w-full border-4 border-yellow-500 shadow-xl space-y-4">
            ...
            <div className="w-40 h-40 mx-auto rounded-full overflow-hidden border-4 border-yellow-500">
              <img src="/image/hidden-artifact.png" alt="Hidden Treasure" className="w-full h-full object-cover" />
            </div>
            <button onClick={() => setShowPopup(false)} className="mt-4 bg-green-600 hover:bg-green-700 px-6 py-2 rounded-lg text-white shadow">
              Đóng
            </button>
          </div>
        </div>
