...
function generateMathQuestion(): MathQuestion {
  ...
  do {
    ...
    // Tính kết quả
    result = operation1 === "+" ? (operation2 === "+" ? a + b + c : a + b - c) : operation2 === "+" ? a - b + c : a - b - c;
...
export default function Game1() {
  ...
  const [lastCollectedPart, setLastCollectedPart] = useState<number | null>(null);
  ...
  const [selectedPlaceInfo, setSelectedPlaceInfo] = useState<Place | null>(null);
  ...
  useEffect(() => {
    if (started && !showConclusion && !waitingToFinish && !showNewPartInfo && !isPaused) {
      ...
      flickerTimerRef.current = window.setInterval(() => {
        ...
        if (Math.random() < 0.15) {
          ...
          setTimeout(() => setFlickerHighlight(false), 200 + Math.random() * 300);
        }
      }, 1500);
    }
  ...
  useEffect(() => {
    ...
    if (started && !showConclusion && !waitingToFinish && !showNewPartInfo && !isPaused && correctCount < 5) {
  ...
  }, [started, showConclusion, waitingToFinish, showNewPartInfo, isPaused, currentIndex, correctCount]);
  ...
  const handleDotClick = (place: Place) => {
    ...
    if (!waitingToFinish && !showNewPartInfo && !isPaused && !showConclusion && currentQuestion) {
  ...
  if (showConclusion) {
    return <Conclusion1 timeUsed={timeUsed} matchedCards={correctCount} totalCards={5} win={correctCount > 0} timeoutCount={timeoutCount} />;
  }
  ...
  return (
    <div className="game-zone flex w-full h-full gap-4 pt-4 px-6 bg-[#0c1e35] font-pirate text-yellow-100">
              ...
              <ul className="space-y-2">
                ...
                <li>
                  <button
                    onClick={togglePause}
                    className="w-full text-left px-3 py-2 hover:bg-amber-800/50 rounded flex items-center gap-2 text-amber-200">
                    {isPaused ? <FaPlay /> : <FaPause />} {isPaused ? "Tiếp tục" : "Tạm dừng"}
                  </button>
                </li>
                <li>
                  <button
      ...
      <div className="relative flex-1 rounded-xl border-4 border-amber-600 shadow-lg overflow-hidden bg-[#0c1e35]">
        {isPaused && (
          <div className="absolute inset-0 bg-black/50 z-10 flex items-center justify-center">
            <div className="text-center bg-[#0f172a]/80 p-6 rounded-xl border-2 border-amber-600">
              <h2 className="text-3xl font-bold text-amber-300 mb-4">Đã tạm dừng</h2>
              <button onClick={togglePause} className="px-4 py-2 bg-amber-600 hover:bg-amber-700 rounded-lg text-white">
        ...
        <img src="/places/map.jpg" alt="Treasure Map" className="absolute w-full h-full object-cover opacity-90" />
        {places.map((place, index) => (
          <motion.div
            key={place.id}
            initial={false}
            animate={{
              left: `${((shuffledPositions[place.id]?.x ?? place.position.x) * 100).toFixed(2)}%`,
              top: `${((shuffledPositions[place.id]?.y ?? place.position.y) * 100).toFixed(2)}%`,
            }}
            transition={{
      ...
      <div className="flex flex-col gap-4 w-[360px]">
        ...
        <div className="bg-[#0c1e35]/90 border-4 border-amber-600 p-4 rounded-lg shadow">
          <div className="text-center">
            <h3 className="text-xl font-bold text-amber-300">Điểm số: {score}</h3>
            {timeoutCount > 0 && (
        ...
        <div className="bg-[#0c1e35]/90 border-4 border-amber-600 p-4 rounded-lg shadow">
          <div className="flex flex-col gap-3 h-[120px]">
            <div className="flex items-center justify-between">
              <div className="w-8 h-8 relative">
                <CircularProgressbarWithChildren
                  ...
                  <span className="text-amber-200 text-[10px] font-bold">{Math.round(shipProgress)}%</span>
                </CircularProgressbarWithChildren>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-wrap items-center justify-center gap-2">
              {!started ? (
                <button onClick={handleStart} className="bg-amber-600 hover:bg-amber-700 px-6 py-2 rounded-lg text-white shadow text-lg font-bold">
                  🏴‍☠️ Bắt đầu!
                </button>
              ) : collectedParts.length > 0 ? (
                collectedParts.map((partIndex) => (
                  <div key={partIndex} className="w-12 h-12 bg-amber-800/50 rounded border border-amber-500" title={shipParts[partIndex].name}>
                    <img src={shipParts[partIndex].image} alt={shipParts[partIndex].name} className="w-full h-full object-contain p-1" />
                  </div>
                ))
              ) : (
        ...
        {started && currentQuestion && !showNewPartInfo ? (
          <div className="bg-[#0c1e35]/90 border-4 border-amber-600 p-4 rounded-lg shadow text-center">
            <div className="mb-4">
              <h3 className="text-xl font-bold text-amber-300 mb-2">Câu hỏi {currentIndex + 1}/5</h3>
              <p className="text-2xl font-bold text-amber-100">{currentQuestion.expression} = ?</p>
            </div>
        ...
        {shipProgress >= 100 && (
          <div className="bg-[#0c1e35]/90 border-4 border-amber-600 p-4 rounded-lg shadow text-center">
            ...
            <p className="text-md font-semibold text-amber-200">Bạn đã hoàn thành con tàu cướp biển!</p>
            <p className="text-sm text-amber-100 mt-1">Đang chuyển đến kết quả...</p>
          </div>
        )}
        ...
        {showNewPartInfo && lastCollectedPart !== null ? (
          <div className="bg-[#1a3d65] border-4 border-amber-600 p-4 rounded-lg shadow text-center space-y-2">
            ...
            <p className="text-lg font-semibold text-amber-100">Bạn đã tìm thấy {shipParts[lastCollectedPart].name}!</p>
            <div className="flex gap-4 justify-center items-center">
              <div className="w-24 h-24 rounded-lg overflow-hidden border-2 border-amber-500 bg-amber-800/30 p-2">
                <img src={shipParts[lastCollectedPart].image} alt={shipParts[lastCollectedPart].name} className="w-full h-full object-contain" />
              </div>
            ...
            <p className="text-amber-200">{shipParts[lastCollectedPart].description}</p>
            <p className="text-sm text-amber-200">Tìm thấy tại: {shipParts[lastCollectedPart].placeName}</p>
            <p className="text-sm text-amber-300 mt-1">Còn {5 - collectedParts.length} mảnh tàu để tìm!</p>
          </div>
        ) : null}
        ...
        {selectedDot ? (
          <div className="bg-[#0c1e35]/90 border-4 border-amber-600 p-4 rounded-lg shadow overflow-y-auto max-h-[65vh] space-y-2">
            ...
            <p className="text-sm text-yellow-200">🛠️ Kiến trúc sư: {selectedDot.architect}</p>
            <p className="text-sm text-yellow-200">📆 Năm xây: {selectedDot.builtYear}</p>
            <p className="text-sm text-yellow-200">📏 Tuổi: {selectedDot.age} năm</p>
            <p className="text-sm text-yellow-100 whitespace-pre-line">{selectedDot.history}</p>
          </div>
        ) : null}

        {!started && selectedPlaceInfo && (
          <div className="bg-[#0c1e35]/90 border-4 border-amber-600 p-4 rounded-lg shadow">
            <h3 className="text-xl font-bold text-amber-300 mb-3 text-center">Thông tin địa điểm: {selectedPlaceInfo.name}</h3>
            <div className="mb-4">
              <img
