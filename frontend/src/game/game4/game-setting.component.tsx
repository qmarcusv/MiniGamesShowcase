...
import { mapComparisons, MapComparison, DifferenceSpot } from "./game4-maps.tsx";

export default function Game4Setting() {
  const [selectedMap, setSelectedMap] = useState<MapComparison | null>(mapComparisons[0]);
  const [editingSpots, setEditingSpots] = useState<DifferenceSpot[]>(selectedMap?.differences || []);
  ...
  return (
    <div className="h-full bg-slate-900 text-white p-4">
      <h1 className="text-3xl font-bold mb-6 text-center">🛠 Cài đặt bản đồ so sánh</h1>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <h2 className="text-lg mb-2 text-center">Ảnh {activeImage === "past" ? "Quá Khứ" : "Hiện Tại"}</h2>
          <div
            ...
            <img
              src={activeImage === "past" ? selectedMap?.pastImage : selectedMap?.currentImage}
              alt={activeImage}
          ...
          <div className="flex justify-center gap-4 mt-4">
            <button
              className={`px-4 py-2 rounded-lg ${activeImage === "past" ? "bg-green-500" : "bg-gray-600"}`}
              onClick={() => setActiveImage("past")}>
              Quá khứ
            </button>
            <button
              className={`px-4 py-2 rounded-lg ${activeImage === "current" ? "bg-green-500" : "bg-gray-600"}`}
        ...
        <div className="bg-white/5 p-4 rounded-xl max-h-[80vh] overflow-y-auto">
          <h3 className="text-xl font-semibold mb-4">📋 Danh sách điểm khác biệt</h3>
          {editingSpots.map((spot) => (
            <div key={spot.id} className="mb-4 bg-white/10 p-3 rounded">
              ...
              <button onClick={() => handleRemove(spot.id)} className="mt-2 text-red-400 text-sm hover:underline">
                Xóa điểm này
              </button>
            </div>
          ))}
