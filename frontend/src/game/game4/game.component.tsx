	...
	if (showConclusion || timeLeft <= 0) {
		...
		return (
			<Conclusion4
				...
				totalCards={selectedMap.differences.length}
				win={allFound || false}
			/>
		);
	}
	...
	return (
		<div className="game-zone  bg-slate-900 text-white p-4">
			<div className="flex justify-between items-center mb-4">
				...
				<h3>
					✅ {foundSpots.length}/{selectedMap.differences.length} điểm đã tìm
					thấy
				</h3>
				{allFound && (
					<button
						onClick={() => setShowConclusion(true)}
						className="bg-emerald-500 px-4 py-2 rounded shadow hover:bg-emerald-600"
					>
						Xem kết quả
					</button>
				)}
			...
			<div className="grid grid-cols-3 gap-4">
				...
				<div className="bg-white/5 rounded-xl p-4 overflow-y-auto max-h-[80vh]">
					<h3 className="text-lg font-semibold mb-2">
						📌 Giải thích các điểm đã tìm thấy:
					</h3>
