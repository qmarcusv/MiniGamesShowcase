  ...
  return (
    <div className="game3-container bg-slate-900 text-white h-full flex p-6 gap-6 items-stretch">
        ...
        <div className="w-full flex-1 rounded-lg bg-white/10 shadow flex items-center justify-center p-4 min-h-[72px]">
          {selectedCard ? (
            <CircularProgressbarWithChildren
              ...
              <div className={`text-sm font-bold ${timer <= 3 ? "text-red-500" : "text-blue-300"}`}>{timer}s</div>
            </CircularProgressbarWithChildren>
          ) : (
            <span className="text-sm text-slate-400 text-center">⏳ Sẵn sàng</span>
          )}
        </div>
      ...
      <div className="flex-1 bg-white/10 rounded-lg p-6 flex flex-col items-center justify-start space-y-6 relative">
        ...
        {/* Word Display */}
        <div className="text-3xl tracking-widest font-mono">{selectedCard ? display : "_ _ _ _"}</div>
        ...
        <div className="grid grid-cols-9 gap-2 max-w-xl">
          {"ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("").map((letter) => (
            <button
              key={letter}
              disabled={guessed.includes(letter) || !selectedCard || fails >= maxFails}
              onClick={() => handleGuess(letter)}
              className={`px-2 py-1 rounded ${guessed.includes(letter) ? "bg-gray-600 text-white" : "bg-blue-500 hover:bg-blue-600"}`}>
              {letter}
            </button>
          ))}
        </div>
        ...
        {/* Hint */}
        {selectedCard && <div className="text-sm text-blue-300 text-center mt-4">Gợi ý: {dummyData[selectedCard]}</div>}
      </div>
    </div>
  );
}
