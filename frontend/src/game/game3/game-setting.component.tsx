	...
	return (
		<div className="min-h-screen bg-slate-900 text-white p-6">
			<div className="max-w-xl mx-auto bg-white/10 p-6 rounded-xl space-y-6">
				<h2 className="text-2xl font-bold text-center text-pink-300">
					Cài đặt từ vựng cho Flowerman
				</h2>
				...
				<div className="space-y-4">
					...
					<button
						onClick={handleAdd}
						className="bg-pink-500 px-4 py-2 rounded hover:bg-pink-600 transition w-full"
					>
						Thêm từ
					</button>
				</div>
				...
				<div className="space-y-2">
					...
					<ul className="list-disc pl-6 space-y-1">
						{words.map((entry, i) => (
							<li key={i}>
								<span className="text-pink-300 font-bold">{entry.word}</span> -{" "}
								{entry.hint}
							</li>
						))}
					</ul>
