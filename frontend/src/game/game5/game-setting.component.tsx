...
export default function Game5Setting() {
	...
	return (
		<div className="p-8 max-w-5xl mx-auto text-left bg-gradient-to-b from-purple-50 to-blue-50 min-h-screen rounded-xl shadow-md">
			...
			<ul className="space-y-4">
				{documents.map((doc, idx) => (
					<li key={doc.id} className="border p-5 rounded-lg bg-white shadow">
						<strong className="block text-lg text-purple-700 mb-2">
							Tài liệu {idx + 1}
						</strong>
						<p>
						...
						<p className="mt-1 text-sm text-gray-500 italic">
							⤷ Dùng làm câu hỏi: {doc.mappedQuestions.join(", ") || "Không có"}
						</p>
					</li>
				))}
			</ul>
